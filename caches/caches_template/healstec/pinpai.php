...
<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=914270efcb9290d88ffb392b7057bb92&action=lists&catid=32&order=id+DESC&num=10\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$data = $content_tag->lists(array('catid'=>'32','order'=>'id DESC','limit'=>'10',));}?>
		...
		<div class="container-fluid tu_o tu_o_2" style="background: url(<?php echo $r['thumb'];?>); width: 100%;  background-size: cover; margin-bottom:2%;">
				...
				<div class="tu_zh row" id="pp">
					...
					<p style="font-size: 18px; margin-top: -1%;" class="col-xs-12"><?php echo $r['description'];?></p>
                        <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=aa1a7c97cbdd96c3fe8a1a375c2bc9b9&sql=select+%2A+from+hl_news_data+where+id%3D%27%24r%5Bid%5D%27&cache=3600&return=data_ca\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$tag_cache_name = md5(implode('&',array('sql'=>'select * from hl_news_data where id=\'$r[id]\'',)).'aa1a7c97cbdd96c3fe8a1a375c2bc9b9');if(!$data_ca = tpl_cache($tag_cache_name,3600)){pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("select * from hl_news_data where id='$r[id]' LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data_ca = $a;unset($a);if(!empty($data_ca)){setcache($tag_cache_name, $data_ca, 'tpl_data');}}?>
					<p class="col-xs-12"><?php echo $data_ca['0']['content'];?></p>
                        <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
					</div>
				</div>	
				</div>        								
...
<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
		...
		<div class="container" style="padding-left: 0; padding-right: 0; margin:2% auto;">
			<ul class="row" class="ro_pp">
                ...
                <?php $n=1; if(is_array($data)) foreach($data AS $k => $r) { ?>
				<li class="col-md-12 col-sm-12 col-xs-12 p_pai"  style="border: 0; padding-left: 0; padding-right: 0;<?php if($n>1) { ?> margin-top: 2%;<?php } ?>"   >
					<a href="<?php echo $r['redirect_url'];?>">
					<div class="col-md-11 col-sm-11 col-xs-11"><p ><?php echo $r['title'];?></p></div>
					<h1 class="glyphicon glyphicon-triangle-bottom col-md-1 col-sm-1 col-xs-1"></h1>					
					</a>
